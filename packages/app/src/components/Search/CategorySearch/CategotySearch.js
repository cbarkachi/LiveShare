...
import { priceInit, calenderItem, getSkill, getTags } from "../SearchParams";
import { getQueryMap } from "library/helpers/url_handler";
import CategroySearchWrapper from "./CategorySearch.style"; // ActionWrapper, // ItemWrapper, // RoomGuestWrapper,

const CategotySearch = ({ history, location }) => {
  const searchParams = getStateFromUrl(location);
  const state = {
    skill: getQueryMap(location.search).get("skill"),
  ...
  const { amenities, property, date_range, price } = state;
  ...
  return (
    <CategroySearchWrapper>
      ...
      {/* <ViewWithPopup
        ...
        key={400}
        noView={true}
        view={<Button type="default">Available Dates</Button>}
        popup={
          <DateRangePickerBox
