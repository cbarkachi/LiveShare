export function getEncodedUrl(urlData) {
  const url = Object.keys(urlData)
    .map((k) => {
      return encodeURIComponent(k) + "=" + encodeURIComponent(urlData[k]);
    })
    .join("&");
  return "?" + url;
...
export function setStateToUrl(state) {
  return getEncodedUrl(state);
}

export function getQueryMap(url) {
  const components = url.split("/");
  const queryMap = new URLSearchParams(components[components.length - 1]);
  return queryMap;
}
